    ...
    return (
        <div className="w-full h-full flex flex-col gap-2">
            <div className="flex flex-col justify-start gap-2">
                ...
                <input name={`old_${name}_${status}`} disabled className="line-through" value={value} onChange={() => { }} />
            </div>

            <div className="flex flex-col justify-start gap-2">
                <label htmlFor={`${name}_${status}`} className="text-sm text-gray-500/70">{name}</label>
                <input name={`${name}_${status}`} className="text-white text-base outline-none" placeholder="Enter accurate value..." value={validData} onChange={(e) => setValidData(e.target.value)} />
            </div>
        </div>
    )
}